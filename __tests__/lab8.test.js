describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let shadow = await prod.getProperty("shadowRoot");

    let button = await shadow.$("button");
    await button.click();
    let buttonText = await button.getProperty("innerText");
    let newbuttonText = await buttonText.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartQuery = await cart.getProperty("innerText");
    const cartText = await cartQuery.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let buttonQuery = await shadow.$("button");
      let button = await buttonQuery.getProperty("innerText");
      let buttonText = await button.jsonValue();

      if (buttonText != "Remove from Cart")
        bool = false;
    }
    expect(bool).toBe(true);
    let countQuery = await page.$('#cart-count');
    let count = await countQuery.getProperty("innerText");
    let countText = await count.jsonValue();
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    let storage = await page.evaluate(async () => {
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let countText = await count.getProperty('innerText');
    let countJson = await countText.jsonValue();
